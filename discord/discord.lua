...
local RECENT_ALERTS_KEY = "plugin_discord_recent_alerts"
...
function discord:incr_unlisted(period)
...
function discord:push_recent_alert(alert)
...
function discord:log(bypass_use_discord)
	...
	if matcher then
		...
		if not match then
			...
			-- The title is completed in the timer once the threshold is reached
			local summary = {
				username = "BunkerWeb",
				embeds = {
					{
			...
			local hdr
			hdr, err = ngx_timer.at(0, self.unlisted, self, summary)
			if not hdr then
...
function discord.unlisted(premature, self, summary)
	...
	summary.embeds[1].title = tostring(count)
		.. " requests from IPs not in DISCORD_ALERT_IPS have been denied within "
		.. tostring(period)
		.. "s"
	discord.send(premature, self, summary)
end
